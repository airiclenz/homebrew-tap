...
| Formula | Description |
|---------|-------------|
| [`apogee`](https://github.com/airiclenz/apogee) | Terminal coding agent built local-first, engineered so small LLMs deliver |
...
`apogee` installs a prebuilt binary for your platform (macOS and Linux × `arm64` and
`amd64`) and needs no Go toolchain; the other two build from source.
...
brew install apogee            # or llama-launcher, screen-transit
```
...
```bash
brew update
brew upgrade apogee            # or llama-launcher, screen-transit
```

## Uninstall

```bash
brew uninstall apogee               # or llama-launcher, screen-transit
